...
.block-container{padding-top:3rem!important;max-width:640px!important;}
.badge-green{display:inline-block;background:#E1F5EE;border:0.5px solid #5DCAA5;border-radius:999px;font-size:12px;color:#085041;padding:4px 14px;letter-spacing:0.03em;margin-bottom:1.25rem;}
.badge-purple{display:inline-block;background:#EEEDFE;border:0.5px solid #AFA9EC;border-radius:999px;font-size:12px;color:#3C3489;padding:4px 14px;letter-spacing:0.03em;margin-bottom:1.25rem;}
.badge-gray{display:inline-block;background:#f5f5f4;border:0.5px solid #e0deda;border-radius:999px;font-size:12px;color:#888;padding:4px 14px;letter-spacing:0.03em;margin-bottom:1.25rem;}
...
.title-green{font-size:22px;font-weight:500;color:#0F6E56;margin-bottom:0.4rem;}
.title-purple{font-size:22px;font-weight:500;color:#534AB7;margin-bottom:0.4rem;}
.card{background:#f5f5f4;border:0.5px solid #e0deda;border-radius:12px;padding:1.25rem 1.5rem;margin-bottom:1.25rem;}
.card-sm{background:#f5f5f4;border:0.5px solid #e0deda;border-radius:12px;padding:1rem 1.25rem;margin-bottom:1rem;}
.card-green{background:#E1F5EE;border:0.5px solid #5DCAA5;border-radius:12px;padding:1rem 1.25rem;margin-bottom:1rem;}
.card-purple{background:#EEEDFE;border:0.5px solid #AFA9EC;border-radius:12px;padding:1rem 1.25rem;margin-bottom:1rem;}
...
.alert-title{font-size:11px;letter-spacing:0.08em;color:#888;margin-bottom:0.6rem;}
.alert-green ul,.alert-purple ul{list-style:none;padding:0;display:flex;flex-direction:column;gap:6px;}
.alert-green li,.alert-purple li{font-size:14px;color:#1a1a1a;line-height:1.55;padding-left:14px;position:relative;}
.alert-green li::before,.alert-purple li::before{content:'—';position:absolute;left:0;color:#aaa;}
.goal-list{display:flex;flex-direction:column;gap:8px;margin-bottom:1.75rem;}
...
.context-card-green{background:#E1F5EE;border:0.5px solid #5DCAA5;border-radius:12px;padding:1rem 1.25rem;margin-bottom:1.75rem;display:flex;align-items:flex-start;gap:12px;}
...
.detail-row{display:flex;align-items:flex-start;gap:8px;font-size:14px;color:#666;line-height:1.5;margin-bottom:6px;}
.detail-row span:first-child{color:#aaa;min-width:10px;}
.section-title{font-size:11px;letter-spacing:0.08em;color:#aaa;margin-bottom:0.9rem;text-transform:uppercase;}
.step-label{font-size:11px;color:#aaa;letter-spacing:0.03em;margin-bottom:1.5rem;}
...
.copy-btn{position:absolute;top:0.75rem;right:0.75rem;font-size:11px;color:#888;background:#fff;border:0.5px solid #e0deda;border-radius:6px;padding:2px 8px;cursor:pointer;transition:all 0.15s;}
div[data-testid="stRadio"] > div[role="radiogroup"]{flex-direction:row!important;gap:2px!important;align-items:center;}
div[data-testid="stRadio"] > div[role="radiogroup"] > label{display:flex!important;flex-direction:column!important;align-items:center!important;gap:3px!important;padding:2px 4px!important;cursor:pointer;min-width:36px;}
div[data-testid="stRadio"] > div[role="radiogroup"] > label > div > p{font-size:10px!important;color:#888!important;margin:0!important;}
...
def save_participant(conv_id, p, style):
    sb.table("condition_counts").update({"count": sb.table("condition_counts").select("count").eq("style", style).single().execute().data["count"] + 1}).eq("style", style).execute()
    sb.table("participants").upsert({
        "conversation_id": conv_id, "genre": p.get("genre"), "age": p.get("age"),
        "achat": p.get("achat"), "pref_achat": p.get("pref_achat"),
        "freq_ia": p.get("freq_ia"), "anciennete": p.get("anciennete"),
        "consulte": p.get("consulte"), "decision": p.get("decision"),
...
def show_p1():
    ...
    st.markdown("""<div class="card">
      ...
      <div style="display:flex;flex-direction:column;gap:10px;">
        ...
        <div style="display:flex;gap:12px;font-size:14px;color:#444;line-height:1.55;"><span style="font-weight:500;color:#1a1a1a;min-width:20px;">4.</span><span>Vous répondez à un <strong>questionnaire</strong>.</span></div>
      </div></div>""", unsafe_allow_html=True)
    st.markdown("""<div class="card-sm" style="font-size:14px;color:#666;line-height:1.65;">
      Il n'y a pas de bonne ou de mauvaise réponse. Toutes vos données sont <strong>entièrement anonymes</strong>.
    </div>""", unsafe_allow_html=True)
    st.markdown("---")
...
def show_p2():
    ...
    achat = st.radio("À quelle fréquence effectuez-vous des achats en ligne ?",
        ["Jamais","Rarement","Parfois","Souvent"],index=None,key="r_achat")
    pref = st.radio("Lorsque vous devez effectuer un achat, vous préférez :",
        ["Principalement en ligne","Autant en ligne qu'en magasin","Principalement en magasin"],
        index=None,key="r_pref")
    c1,c2=st.columns([1,3])
    with c1:
        if st.button("← Retour"): st.session_state["screen"]="p1"; st.rerun()
    with c2:
        if st.button("Suivant →", type="primary"):
            if not achat or not pref: st.error("Veuillez répondre à toutes les questions.")
            else:
                st.session_state["profil"].update({"achat":achat,"pref_achat":pref})
...
def show_p3():
    ...
    anc = st.radio("Depuis combien de temps utilisez-vous des agents conversationnels ?",
        ["Je n'en utilise pas","Moins de 6 mois","6 mois – 1 an","1 – 2 ans","Plus de 2 ans"],index=None,key="r_anc")
    cons = st.radio("Avez-vous déjà consulté un agent IA avant de faire un achat ?",
        ["Oui","Non"],index=None,horizontal=True,key="r_cons")
    dec = st.radio("Avez-vous déjà basé une décision d'achat sur une recommandation de l'IA ?",
    ...
    with c2:
        if st.button("Suivant →", type="primary"):
            if not freq_ia or not anc or not cons or not dec: st.error("Veuillez répondre à toutes les questions.")
            else:
                st.session_state["profil"].update({"freq_ia":freq_ia,"anciennete":anc,"consulte":cons,"decision":dec})
...
def show_p4():
    ...
    st.markdown("""<div class="consent-box">
      <p style="font-size:14px;color:#666;line-height:1.65;margin-bottom:1rem;">
        Cette étude est réalisée dans le cadre d'un mémoire de fin de master. Votre participation est entièrement <strong>volontaire</strong> et anonyme. Les données ne seront jamais associées à votre identité.
      </p>
      <p style="font-size:14px;color:#666;line-height:1.65;">Vous êtes libre de vous retirer à tout moment.</p>
    </div>""", unsafe_allow_html=True)
...
# PAGES MISSION
# ════════════════════════════════════════════
EP_EXAMPLE = "J'organise une soirée chez moi ce week-end. Je voudrais un repas maison. Qu'est-ce que tu me proposes et quels ingrédients dois-je acheter ?"
MED_EXAMPLE = "Mon amie Julie a très mal au bas du dos depuis ce matin. Elle prend déjà de l'aspirine pour le cœur. Que peut-elle prendre pour soulager la douleur ?"

def copy_component(text, btn_id):
    text_js = text.replace('"', '\\"').replace("'", "\\'")
    return f"""<div style="border:0.5px solid #e0deda;border-radius:12px;padding:1rem 1.25rem;position:relative;font-family:system-ui,sans-serif;margin-bottom:2rem;">
      <div style="font-size:11px;letter-spacing:0.08em;color:#aaa;margin-bottom:0.65rem;">EXEMPLE POUR COMMENCER</div>
      <button id="btn_{btn_id}" onclick="copy_{btn_id}()" style="position:absolute;top:0.75rem;right:0.75rem;font-size:11px;color:#888;background:#fff;border:0.5px solid #e0deda;border-radius:6px;padding:2px 8px;cursor:pointer;">Copier</button>
      <div style="background:#f5f5f4;border-radius:0 12px 12px 12px;padding:0.85rem 1rem;font-size:14px;color:#1a1a1a;line-height:1.6;font-style:italic;">{text}</div>
    </div>
    <script>function copy_{btn_id}(){{var ta=document.createElement('textarea');ta.value="{text_js}";ta.style.position='fixed';ta.style.opacity='0';document.body.appendChild(ta);ta.focus();ta.select();document.execCommand('copy');document.body.removeChild(ta);var b=document.getElementById('btn_{btn_id}');b.textContent='Copié !';b.style.color='#1D9E75';b.style.borderColor='#1D9E75';setTimeout(function(){{b.textContent='Copier';b.style.color='#888';b.style.borderColor='#e0deda';}},2000);}}</script>"""
...
def show_mission_ep():
    ...
    st.markdown('<p style="font-size:15px;color:#666;margin-bottom:1.5rem;">Préparez votre soirée avec l\'aide d\'un assistant d\'achat.</p>', unsafe_allow_html=True)
    ...
    st.markdown("""<div class="goal-list">
    ...
    components.html(copy_component(f"« {EP_EXAMPLE} »", "ep1"), height=160)
...
def show_mission_med():
    ...
    st.markdown('<p style="font-size:15px;color:#666;margin-bottom:1.5rem;">Trouvez une solution adaptée pour soulager votre amie.</p>', unsafe_allow_html=True)
    st.markdown("""<div class="context-card-purple">
      <div class="context-header"><div class="context-icon">💊</div><strong style="color:#3C3489;">Contexte : la situation de Julie</strong></div>
    ...
    components.html(copy_component(f"« {MED_EXAMPLE} »", "med1"), height=160)
    if st.button("J'ai compris →", type="primary", use_container_width=True):
...
# ════════════════════════════════════════════
RAPPEL = {
    "epicerie": """<div class="context-card-green" style="display:flex;align-items:flex-start;gap:12px;">
      <span style="font-size:18px;flex-shrink:0;">🏠</span>
      <p style="font-size:14px;color:#085041;line-height:1.55;"><strong>Contexte :</strong> soirée chez vous ce week-end — repas fait maison (apéritif, plat, dessert).</p>
    </div>
    <div class="section-title">VOTRE OBJECTIF</div>
    <div class="goal-list">
      <div class="goal-item"><span class="goal-num">1.</span>Trouver des idées de repas complet</div>
      <div class="goal-item"><span class="goal-num">2.</span>Identifier les ingrédients nécessaires</div>
    </div>
    <div class="alert-green"><div class="alert-title">À SAVOIR</div><ul>
      <li>Si une proposition vous convient, demandez à l'assistant d'ajouter les ingrédients au panier.</li>
      <li>Cliquez <strong>« Passer à l'étape suivante »</strong> une fois votre panier constitué.</li>
    </ul></div>""",
    "medicaments": """<div class="context-card-purple">
      <div class="context-header"><div class="context-icon">💊</div><strong style="color:#3C3489;">La situation de Julie</strong></div>
      <div class="context-details">
        <div class="detail-row"><span>—</span><span>45 ans, forte douleur bas du dos depuis ce matin</span></div>
        <div class="detail-row"><span>—</span><span>Ne peut pas consulter avant 2 semaines</span></div>
        <div class="detail-row"><span>—</span><span>Aspirine 100 mg/jour pour le cœur</span></div>
      </div>
    </div>
    <div class="section-title">VOTRE OBJECTIF</div>
    <div class="goal-list">
      <div class="goal-item"><span class="goal-num">1.</span>Identifier une solution adaptée</div>
      <div class="goal-item"><span class="goal-num">2.</span>Vérifier la compatibilité avec sa situation</div>
    </div>
...
def show_chat(produit):
    ...
    badge = '<div class="chat-badge-green"><div style="width:7px;height:7px;border-radius:50%;background:#1D9E75;display:inline-block;margin-right:4px;"></div>ÉTAPE 1 / 2 — ASSISTANT COURSES</div>' if produit=="epicerie" \
        else '<div class="chat-badge-purple"><div style="width:7px;height:7px;border-radius:50%;background:#7F77DD;display:inline-block;margin-right:4px;"></div>ÉTAPE 2 / 2 — ASSISTANT PHARMACEUTIQUE</div>'
    st.markdown(badge, unsafe_allow_html=True)

    with st.expander("📋 Rappel de l'étape en cours", expanded=False):
        st.markdown(RAPPEL[produit], unsafe_allow_html=True)
        example_text = EP_EXAMPLE if produit=="epicerie" else MED_EXAMPLE
        btn_id = "ep2" if produit=="epicerie" else "med2"
        components.html(copy_component(f"« {example_text} »", btn_id), height=160)
...
def show_questionnaire():
    st.markdown("""<style>
div[data-testid="stRadio"]>div[role="radiogroup"]{flex-direction:row!important;gap:2px!important;}
div[data-testid="stRadio"]>div[role="radiogroup"]>label{display:flex!important;flex-direction:column!important;align-items:center!important;gap:3px!important;padding:2px 4px!important;min-width:36px;}
div[data-testid="stRadio"]>div[role="radiogroup"]>label>div>p{font-size:10px!important;color:#888!important;margin:0!important;}
</style>""", unsafe_allow_html=True)
    ...
    with st.form("q_form"):
        ...
        for section_label, questions in [
            ...
            for n, text in questions:
                st.markdown(f"""<div style="background:#fff;border:0.5px solid #ddd;border-radius:12px;padding:1rem 1.25rem;margin-bottom:4px;">
                  <p style="font-size:14px;line-height:1.6;color:#1a1a1a;"><span style="font-size:12px;color:#888;margin-right:6px;">{n}.</span>{text}</p>
                </div>""", unsafe_allow_html=True)
                ...
                with c2:
                    ep = st.radio("e",OPTS,index=None,horizontal=True,label_visibility="collapsed",key=f"q{n}_ep")
                ...
                with c4:
                    med = st.radio("m",OPTS,index=None,horizontal=True,label_visibility="collapsed",key=f"q{n}_med")
        ...
        if st.form_submit_button("Envoyer mes réponses →", type="primary", use_container_width=True):
            if any(v is None for v in results.values()):
                ...
                st.session_state["screen"] = "fin"; st.rerun()
...
def show_fin():
    st.markdown("""<div style="text-align:center;padding:3rem 0;">
      ...
      <p style="color:#666;font-size:15px;margin-top:1rem;">Merci pour votre contribution.</p>
