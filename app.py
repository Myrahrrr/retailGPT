...
CONDITIONS = [{"style":"machine_like"},{"style":"human_like_formal"},{"style":"human_like_friendly"}]
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS participants (
        conversation_id TEXT PRIMARY KEY, genre TEXT, age TEXT, achat TEXT, pref_achat TEXT,
        freq_ia TEXT, anciennete TEXT, consulte TEXT, decision TEXT, style TEXT, timestamp TEXT)""")
    c.execute("""CREATE TABLE IF NOT EXISTS condition_counts (
        style TEXT PRIMARY KEY, count INTEGER DEFAULT 0, completed INTEGER DEFAULT 0)""")
    c.execute("""CREATE TABLE IF NOT EXISTS questionnaire (
        conversation_id TEXT PRIMARY KEY,
        q1_ep INT,q2_ep INT,q3_ep INT,q4_ep INT,q5_ep INT,q6_ep INT,
        q7_ep INT,q8_ep INT,q9_ep INT,q10_ep INT,q11_ep INT,q12_ep INT,
        q1_med INT,q2_med INT,q3_med INT,q4_med INT,q5_med INT,q6_med INT,
        q7_med INT,q8_med INT,q9_med INT,q10_med INT,q11_med INT,q12_med INT,
        timestamp TEXT)""")
    for cond in CONDITIONS:
        c.execute("INSERT OR IGNORE INTO condition_counts VALUES (?,0,0)",(cond["style"],))
    conn.commit(); conn.close()

def assign_condition():
    conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    c.execute("SELECT style FROM condition_counts ORDER BY completed ASC, count ASC, style ASC LIMIT 1")
    row = c.fetchone(); conn.close()
    return row[0] if row else "human_like_formal"
...
def save_participant(conv_id, p, style):
    ...
    c.execute("INSERT OR REPLACE INTO participants VALUES (?,?,?,?,?,?,?,?,?,?,?)",
        (conv_id,p.get("genre"),p.get("age"),p.get("achat"),p.get("pref_achat"),p.get("freq_ia"),
         p.get("anciennete"),p.get("consulte"),p.get("decision"),style,datetime.now().isoformat()))
    conn.commit(); conn.close()

def save_questionnaire(conv_id, r):
    conn = sqlite3.connect(DB_PATH); c = conn.cursor()
    c.execute("""INSERT OR REPLACE INTO questionnaire VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(
        conv_id,
        r.get("q1_ep"),r.get("q2_ep"),r.get("q3_ep"),r.get("q4_ep"),r.get("q5_ep"),r.get("q6_ep"),
        r.get("q7_ep"),r.get("q8_ep"),r.get("q9_ep"),r.get("q10_ep"),r.get("q11_ep"),r.get("q12_ep"),
        r.get("q1_med"),r.get("q2_med"),r.get("q3_med"),r.get("q4_med"),r.get("q5_med"),r.get("q6_med"),
        r.get("q7_med"),r.get("q8_med"),r.get("q9_med"),r.get("q10_med"),r.get("q11_med"),r.get("q12_med"),
        datetime.now().isoformat()))
    c.execute("UPDATE condition_counts SET completed=completed+1 WHERE style=(SELECT style FROM participants WHERE conversation_id=?)",(conv_id,))
    conn.commit(); conn.close()
...
def show_admin():
    ...
    df_rr = pd.read_sql_query(
        "SELECT style, count as assignations, completed as completes FROM condition_counts ORDER BY style", conn)
    st.dataframe(df_rr, use_container_width=True)
    st.divider()

    # ── Participants complétés uniquement ──
    df = pd.read_sql_query("""
        SELECT
            ...
            p.consulte,
            p.decision,
            q.q1_ep,  q.q2_ep,  q.q3_ep,  q.q4_ep,
            q.q5_ep,  q.q6_ep,  q.q7_ep,  q.q8_ep,
            q.q9_ep,  q.q10_ep, q.q11_ep, q.q12_ep,
            q.q1_med, q.q2_med, q.q3_med, q.q4_med,
            q.q5_med, q.q6_med, q.q7_med, q.q8_med,
            q.q9_med, q.q10_med,q.q11_med,q.q12_med,
            q.timestamp as date_questionnaire
        FROM participants p
        INNER JOIN questionnaire q ON p.conversation_id = q.conversation_id
        ORDER BY q.timestamp DESC
    """, conn)
    conn.close()

    # colonnes renommées pour affichage
    rename = {
    ...
    st.subheader(f"{len(df)} participants avec questionnaire complété")
    st.dataframe(df.rename(columns=rename), use_container_width=True)

    # ── Export CSV ──
    st.download_button("Telecharger CSV", df.to_csv(index=False).encode("utf-8"),
    ...
    if st.session_state.get("confirm_reset"):
        st.warning("Êtes-vous sûr ? Cette action supprime toutes les données collectées.")
        c1, c2 = st.columns(2)
        with c1:
            if st.button("Confirmer la suppression", type="primary"):
                conn2 = sqlite3.connect(DB_PATH)
                c = conn2.cursor()
                c.execute("DELETE FROM questionnaire")
                c.execute("DELETE FROM participants")
                c.execute("UPDATE condition_counts SET count=0, completed=0")
